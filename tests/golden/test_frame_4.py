...
expanded edit row anchors the body (spec §8.2), the empty buffer renders the
``AT-T`` default as ghost text with a reverse-video cursor over the first ghost
character, and the footer omits the submit affordance while the buffer is still
ghost-only (spec §6.6). Geometry uses pyte ``screen.display``; the reverse-video
cursor uses pyte cell attributes; SGR 2 (dim ghost) is checked in raw ANSI.
"""
...
# ── geometry: anchored single-source edit block ──────────────────────────────

def test_frame_is_seven_lines_ending_at_footer(frame_4_lines):
    # header, prompt, blank, table header, 1 edit row, blank separator, footer.
    assert len(frame_4_lines) == 7
...
def test_footer_is_two_rows_below_the_single_edit_row(display):
    assert display[5].strip() == ""  # blank separator
    assert "type to edit" in display[6]
