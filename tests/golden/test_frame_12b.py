"""
TASK-007 — golden render test for frame 12b (source pointer on the second source).

From the empty-buffer edit on the APPLE mapping (frame 9) the reviewer presses
``↑``: focus moves to the source list at the *last* active source
``user_symbol: "APPLE"`` and the buffer autofills to ``APPLE``. ``APPLE`` is a
valid commodity token, so the ``✓`` icon renders; the source pointer ``▸`` appears
on the second source row and the row cursor is suppressed (spec §7.4 / §6.3 /
FR21).
"""
...
def test_frame_is_eleven_lines_ending_at_footer(frame_12b_lines):
    assert len(frame_12b_lines) == 11
...
def test_buffer_autofilled_from_last_source(display):
    # Last active source user_symbol: "APPLE" -> buffer "APPLE" at the token field.
    assert display[4][7:12] == "APPLE"


def test_valid_icon_two_columns_right_of_the_cursor(display):
    # Cursor block at col 12; ✓ at cursor + 2 = col 14.
    assert display[4][14] == "✓"
    assert "✗" not in display[4]


def test_reverse_video_cursor_block_after_buffer(frame_12b_screen):
    assert frame_12b_screen.buffer[4][12].reverse is True


# ── source pointer (spec §7.4 / §6.3) ────────────────────────────────────────

def test_source_pointer_marks_the_second_source(display):
    # ▸ sits in the source-pointer column (7+W+M = 33, index 32) on the second
    # source row, next to user_symbol: "APPLE".
    assert display[5][32] == "▸"
    assert display[5].rstrip().endswith('┃ user_symbol: "APPLE"')


def test_first_source_has_no_pointer(display):
    assert display[4][32] != "▸"
...
def test_context_rows_are_present_and_super_dim(display, frame_12b_lines):
    assert display[6].split()[0] == "4"
    assert display[7].split()[0] == "10"
    assert display[8].split()[0] == "11"
    for i in (6, 7, 8):
        assert frame_12b_lines[i].startswith("\x1b[2m")
