...
``!`` from both the edited row and the preceding context row, and the footer
gains the submit hint (spec §6.6).
"""
...
# ── live collision recompute clears both AT-T markers ────────────────────────

def test_preceding_context_row_drops_its_collision_marker(display):
    # Row 2 no longer collides once row 3's live buffer is "ATT".
    assert display[4].split()[0] == "2"
    assert display[4][6] == " "
...
def test_reverse_video_cursor_is_a_block_after_the_buffer(frame_5_screen):
    ...
    assert frame_5_screen.display[5][10] == " "
